    ...
    public static class AssetModifier
    {
        ...
        private static void Postprocess()
        {
            ...
            foreach (var guid in AssetDatabase.FindAssets("t:Texture", folders))
                OnPostprocessTexture(AssetImporter.GetAtPath(AssetDatabase.GUIDToAssetPath(guid)) as TextureImporter);

            foreach (var guid in AssetDatabase.FindAssets("t:AudioClip", folders))
                OnPreprocessAudio(AssetImporter.GetAtPath(AssetDatabase.GUIDToAssetPath(guid)) as AudioImporter);

            foreach (var guid in AssetDatabase.FindAssets("t:Model", folders))
                OnPreprocessModel(AssetImporter.GetAtPath(AssetDatabase.GUIDToAssetPath(guid)) as ModelImporter);
        }
