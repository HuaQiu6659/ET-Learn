...
namespace Lin.Editor.Helper
{
    public static class ImporterHelper
    {
        ...
        public static void SetUserData<T>(this AssetImporter self, string key, T value, bool saveAndReimport = false) => self.SetUserData(key, JsonConvert.SerializeObject(value), saveAndReimport);
        ...
        public static void SetUserData(this AssetImporter self, string key, string value, bool saveAndReimport = false)
        {
            ...
            self.userData = jObject.ToString();
        ...
        private const string TAG_KEY = nameof(TAG_KEY);

        public static void AddUserTag(this AssetImporter self, string tag, bool saveAndReimport = false)
        {
            var tags = self.GetUserData<List<string>>(TAG_KEY)??new List<string>();
            if (tags.Contains(tag))
                return;

            tags.Add(tag);
            self.SetUserData(TAG_KEY, tags, saveAndReimport);
        }
        ...
        {
            var tags = self.GetUserData<List<string>>(TAG_KEY);
            if (tags is null)
                return false;
        ...
        public static AssetSummary GetDescription(this AssetImporter self) => AssetSummaryArchiver.GetInstance().GetSummary(self);
